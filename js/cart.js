...
async function getselections() {
  ...
  products_selctions +=
    '<option id="" value="" name="">جاري التحميل...</option>';
...
`;
  for (let i = 0; i < cart.length; i++) {
    ...
    HTMLtable += `
        ...
        <tr>
            <td><input class="text_intput text_input_small" type="text" value="${
              cart[i].title
            }"></input></td>
            <td><input class="text_intput number_input_small mr-t" id="${ide}" onchange="amoutnUpdataCart(${i})" min="1" type="number" value="${
      cart[i].add_amount || 0
    }"></td>
            <td>
            <input class="text_intput number_input_small mr-t " type="number" value="${
              cart[i].price
            }" readonly>
                <span >ريال</span>
            </td>
